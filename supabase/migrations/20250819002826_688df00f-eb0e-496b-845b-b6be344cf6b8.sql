-- Create user roles and enhanced analytics system
...
-- Update participants table to track entries better
ALTER TABLE public.participants 
ADD COLUMN giveaway_id_email_store_key TEXT GENERATED ALWAYS AS (giveaway_id::text || '_' || email || '_' || (SELECT store_id FROM public.giveaways WHERE id = giveaway_id)::text) STORED;

-- Create unique constraint for one entry per email per giveaway per store
CREATE UNIQUE INDEX idx_participants_giveaway_email_store ON public.participants(giveaway_id_email_store_key);

-- Add purchase_id to track which purchase generated entries
ALTER TABLE public.participants 
...
BEGIN
  ...
  LOOP
    ...
    -- Insert participant entry (will fail silently if already exists due to unique constraint)
    ...
    ON CONFLICT (giveaway_id_email_store_key) DO UPDATE SET
